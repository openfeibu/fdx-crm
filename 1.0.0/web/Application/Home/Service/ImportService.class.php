...
class ImportService extends PSIBaseService
{
   ...
   * 物料类型 - 名称转为编码
   */
  private function mtypeNameToCode($name)
  {
    switch ($name) {
      case "原材料":
        return 1000;
  ...
  public function importGoodsFromExcelFile($params)
  {
    ...
    $insertSql = "insert into t_goods (id, code, name, spec, category_id, unit_id, sale_price,	py, 
					            purchase_price, bar_code, data_org, memo, spec_py, m_type) values";
    $dataSql = "('%s', '%s', '%s', '%s', '%s', '%s', %f, '%s', %f, '%s', '%s', '%s', '%s', %d),";
    /**
     * 单元格定义
     * A 商品分类编码
     * B 商品编码
