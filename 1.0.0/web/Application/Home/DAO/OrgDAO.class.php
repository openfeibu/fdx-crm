...
class OrgDAO extends PSIBaseExDAO
{
  ...
  public function addOrg(&$params)
  {
    ...
    // 检查编码是否已经存在
    $sql = "select count(*) as cnt from t_org where org_code = '%s' ";
    $data = $db->query($sql, $orgCode);
    $cnt = $data[0]["cnt"];
    if ($cnt > 0) {
      return $this->bad("编码为{$orgCode}的组织机构已经存在");
    }
  ...
  public function updateOrg(&$params)
  {
    ...
    // 检查编码是否已经存在
    $sql = "select count(*) as cnt from t_org where org_code = '%s' and id <> '%s' ";
  ...
  public function orgWithDataOrg($params)
  {
    ...
    $queryParams = [];
    $ds = new DataOrgDAO($db);
    $rs = $ds->buildSQL(FIdConst::USER_MANAGEMENT_ORGS, "t_org", $loginUserId);
    if ($rs) {
      $sql .= " where " . $rs[0];
      $queryParams = $rs[1];
    }
