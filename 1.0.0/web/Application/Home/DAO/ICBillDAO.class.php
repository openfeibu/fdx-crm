...
class ICBillDAO extends PSIBaseExDAO
{
  ...
  public function commitICBill(&$params)
  {
    ...
    foreach ($items as $i => $v) {
      ...
      if (!$data) {
        $index = $i + 1;
        return $this->bad("第{$index}条记录的物料不存在，无法完成提交");
      }

      if ($goodsCount < 0) {
        $index = $i + 1;
        return $this->bad("第{$index}条记录的物料盘点后库存数量不能为负数");
      }
      if ($goodsMoney < 0) {
        $index = $i + 1;
        return $this->bad("第{$index}条记录的物料盘点后库存金额不能为负数");
      }
      if ($goodsCount == 0) {
        if ($goodsMoney != 0) {
          $index = $i + 1;
          return $this->bad("第{$index}条记录的物料盘点后库存数量为0的时候，库存金额也必须为0");
        }
      }
