...
class PWBillDAO extends PSIBaseExDAO
{
  ...
  public function addPWBill(&$bill)
  {
    ...
    $limitGoods = $warehouse["limitGoods"] == 1;
    $usageType = $warehouse["usageType"];
    ...
    foreach ($items as $i => $item) {
      ...
      if ($limitGoods) {
        ...
        if ($usageType == 10) {
          // 原材料库
          if ($mType != 1000) {
            return $this->bad("当前仓库的用途是原材料库，第{$recordInde}条记录中的物料不是原材料，不能保存");
          }
        } else if ($usageType == 20) {
          // 半成品库
          if ($mType != 2000) {
            return $this->bad("当前仓库的用途是半成品库，第{$recordInde}条记录中的物料不是半成品，不能保存");
          }
        } else if ($usageType == 30) {
          // 产成品库
          if ($mType != 3000) {
            return $this->bad("当前仓库的用途是产成品库，第{$recordInde}条记录中的物料不是产成品，不能保存");
          }
        } else if ($usageType == 40) {
          // 商品库
          if ($mType != 4000) {
            return $this->bad("当前仓库的用途是商品库，第{$recordInde}条记录中的物料不是商品，不能保存");
          }
        } else {
          // bug
      ...
      if (!$supplierDAO->goodsIdIsInGoodsRange($supplierId, $goodsId)) {
        $recordInde = $i + 1;
        return $this->bad("第{$recordInde}条记录中的物料不在当前供应商的关联物料内，不能保存");
      }

      // 关于入库数量为什么允许填写0：
      // 当由采购订单生成采购入库单的时候，采购订单中有多种物料，但是是部分到货
      // 那么就存在有些物料的数量是0的情形。
