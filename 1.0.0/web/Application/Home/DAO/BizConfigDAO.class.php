...
class BizConfigDAO extends PSIBaseExDAO
{
   ...
   * 启用Lodop打印
   */
  private function getEnableLodop(string $id): string
  {
    if ($id == "0") {
      return "不启用";
    } else {
  ...
  public function allConfigs($params)
  {
    ...
    foreach ($data as $v) {
      ...
      if ($id == "1001-01") {
        ...
        $displayValue = $this->getEnableLodop($v["value"]);
      } else {
   ...
   * 是否启用Lodop打印
   */
  public function getEnableLodopFlag(string $companyId): string
  {
    $db = $this->db;
