...
class UserDAO extends PSIBaseExDAO
{
  ...
  public function updateUser($params)
  {
    ...
    if ($dataOrg != $oldDataOrg) {
      // 数据域发生了变化，记录用户数据域变更历史记录
      $sql = "insert into t_user_dataorg_history(id, user_id, old_data_org, new_data_org, update_dt)
              values ('%s', '%s', '%s', '%s', now())";
      $rc = $db->execute($sql, $this->newId(), $id, $oldDataOrg, $dataOrg);
      if ($rc === false) {
        return $this->sqlError(__METHOD__, __LINE__);
      }
    }
  ...
  public function queryData($params)
  {
    ...
    $rs = $ds->buildSQL(FIdConst::USER_MANAGEMENT_USERS, "t_user", $loginUserId);
    if ($rs) {
      $sql .= " and " . $rs[0];
