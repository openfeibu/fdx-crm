...
class SRBillDAO extends PSIBaseExDAO
{
  ...
  public function commitSRBill(&$params)
  {
    ...
    $sql = "select goods_id, convert(rejection_goods_count, $fmt) as rejection_goods_count, 
              inventory_money, wsbilldetail_id
            from t_sr_bill_detail
            where srbill_id = '%s'
            order by show_order";
    ...
    foreach ($items as $index => $v) {
      ...
      // 更新对应的销售出库单明细中的退货数量和实际出库数量
      $sql = "select convert(rej_goods_count, $fmt) as rej_goods_count,
                convert(goods_count, $fmt) as goods_count,
                convert(real_goods_count, $fmt) as real_goods_count,
                sobilldetail_id
              from t_ws_bill_detail
              where id = '%s' ";
      $d = $db->query($sql, $wsBillDetailId);
      if (!$d) {
        // 对应的销售出库单不存在, bug
        $idx = $index + 1;
        return $this->bad("退货明细中第{$idx}条记录在销售出库单中没有对应的记录");
      }
      $wsGoodsCount = $d[0]["goods_count"];
      $wsRejCount = $d[0]["rej_goods_count"];
      $wsRealCount = $d[0]["real_goods_count"];
      $soBillDetailId = $d[0]["sobilldetail_id"];
      if (($wsRejCount + $wsRealCount) != $wsGoodsCount) {
        //数据存在异常，原因是real_goods_count是后加的，历史数据是NULL
        $idx = $index + 1;
        return $this->bad("退货明细中第{$idx}条记录对应的销售出库明细记录的real_goods_count数据异常");
      }
      $wsRejCount += $rejCount;
      $wsRealCount = $wsGoodsCount - $wsRejCount;

      $sql = "update t_ws_bill_detail
              set rej_goods_count = convert(%f, $fmt),
                  real_goods_count = convert(%f, $fmt) 
              where id = '%s' ";
      $rc = $db->execute($sql, $wsRejCount, $wsRealCount, $wsBillDetailId);
      if ($rc === false) {
        return $this->sqlError(__METHOD__, __LINE__);
      }

      // 当前退货入库单对应的销售出库单如果是由销售订单生成的
      // 则需要更新该销售订单中的出库数量和未出库数量
      if ($soBillDetailId) {
        $sql = "select convert(goods_count, $fmt) as goods_count,
                ...
                where id = '%s' ";
        $d = $db->query($sql, $soBillDetailId);
        if ($d) {
          $scBillDetailId = $d[0]["scbilldetail_id"]; // 销售合同明细记录id
          $gc = $d[0]["goods_count"];
          $wc = $d[0]["ws_count"];
          $wc -= $rejCount; // 销售订单明细中的已出库数量
          $lc = $gc - $wc; // 销售订单明细中的未出库数量

          $sql = "update t_so_bill_detail
                  set ws_count = convert(%f, $fmt),
                      left_count = convert(%f, $fmt)
                  where id = '%s' ";
          $rc = $db->execute($sql, $wc, $lc, $soBillDetailId);
          if ($rc === false) {
            return $this->sqlError(__METHOD__, __LINE__);
          }

          // 如果该销售订单是由销售合同创建的
          // 则需要更新销售合同中的订单已执行数量和未执行数量
          if ($scBillDetailId) {
            $sql = "select convert(goods_count, $fmt) as goods_count,
                      convert(so_count, $fmt) as so_count 
                    from t_sc_bill_detail
                    where id = '%s' ";
            $d = $db->query($sql, $scBillDetailId);
            if ($d) {
              $scCount = $d[0]["goods_count"];
              $soCount = $d[0]["so_count"];
              $soCount -= $rejCount;
              $leftCount = $scCount - $soCount;
              $sql = "update t_sc_bill_detail
                      set so_count = convert(%f, $fmt),
                        left_count = convert(%f, $fmt)
                      where id = '%s' ";
              $rc = $db->execute($sql, $soCount, $leftCount, $scBillDetailId);
              if ($rc === false) {
                return $this->sqlError(__METHOD__, __LINE__);
              }
            }
    ...
    if ($paymentType == 0) {
       ...
       * 冲销 应收账款总账
    	 */
	    $minusRejectionSaleMoney = -$rejectionSaleMoney;
	    ...
	    if (!$data) {
		    ...
		    $rvMoney += $minusRejectionSaleMoney;
		    $balanceMoney += $minusRejectionSaleMoney;
	    ...
	    $sql = "insert into t_receivables_detail(id, rv_money, act_money, balance_money, ca_id, ca_type,
              ...
              values ('%s', %f, 0, %f, '%s', 'customer', '%s', now(), '%s', '销售退货入库', '%s')";
	    $rc = $db->execute(
		    $sql,
		    $this->newId(),
		    $minusRejectionSaleMoney,
    ...
    // 把对应的销售出库单的状态改为：2000 - 部分退货，3000 - 全部退货
    $sql = "select count(*) as cnt 
            from t_ws_bill_detail 
            where wsbill_id = '%s' and convert(real_goods_count, $fmt) > 0";
    ...
    $sql = "update t_ws_bill
            set bill_status = %d
            where id = '%s' ";
    $rc = $db->execute($sql, $billStatus, $wsBillId);
    if ($rc === false) {
      return $this->sqlError(__METHOD__, __LINE__);
    }

    $sql = "select so_id from t_so_ws where ws_id ='%s' ";
    $data = $db->query($sql, $wsBillId);
    if ($data) {
      ...
      $d = $db->query($sql, $soBillId);
      $billStatus = $d[0]["bill_status"];
      if ($billStatus >= 4000) {
        // 订单关闭的时候，就不调整销售订单的状态
        // do nothing
      } else {
        // 把对应的销售订单的状态改为：2000 - 部分出库，3000 - 全部出库
        $sql = "select count(*) as cnt 
                from t_so_bill_detail 
                where sobill_id = '%s' and convert(left_count, $fmt) > 0";
