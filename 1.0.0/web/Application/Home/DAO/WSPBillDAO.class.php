...
class WSPBillDAO extends PSIBaseExDAO
{
  ...
  public function addWSPBill(&$bill)
  {
    ...
    foreach ($items as $showOrder => $v) {
      ...
      if ($cnt == 0) {
        ...
        return $this->bad("第{$rowIndex}记录中的物料没有子物料，不能做拆分业务");
      }

      // 检查拆分数量
      if ($goodsCount <= 0) {
        $rowIndex = $showOrder + 1;
        return $this->bad("第{$rowIndex}记录中的物料的拆分数量需要大于0");
      }
  ...
  public function commitWSPBill(&$params)
  {
    ...
    foreach ($items as $showOrder => $v) {
      ...
      if (!$data) {
        return $this->bad("第{$recordIndex}条物料没有库存，无法完成拆分操作");
      }
      $v = $data[0];
      ...
      if ($goodsCount > $balanceCount) {
        return $this->bad("第{$recordIndex}条物料库存量($balanceCount)小于拆分量($goodsCount)，无法完成拆分操作");
      }
  ...
  public function genWSPBillFromPWBillAndCommit($pwBillId, $loginUserId, &$wspBillRef)
  {
    ...
    // 检查采购入库单里面物料有没有子物料
    // 如果所有的物料都没有子物料，即使是expandByBOM == 1也不需要生成拆分单
