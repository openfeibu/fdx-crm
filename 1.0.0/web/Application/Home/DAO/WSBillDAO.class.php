...
class WSBillDAO extends PSIBaseExDAO
{
  ...
  public function wsBillDetailList($params)
  {
    ...
    $sql = "select d.id, g.code, g.name, g.spec, u.name as unit_name, 
              ...
              convert(d.rej_goods_count, $fmt) as rej_goods_count,
              convert(d.real_goods_count, $fmt) as real_goods_count
  ...
  public function addWSBill(&$bill)
  {
    ...
    $sql = "insert into t_ws_bill_detail (id, date_created, goods_id,
              ...
              tax_rate, tax, money_with_tax, goods_price_with_tax, real_goods_count)
            values ('%s', now(), '%s', convert(%f, $fmt), %f, %f, %d, '%s', '%s', '%s', '%s', '%s', '%s',
              %d, %f, %f, %f, convert(%f, $fmt))";
    ...
    if ($sobillRef) {
      // 从销售订单生成销售出库单
      $sql = "select id, company_id, input_user_id, biz_user_id, data_org from t_so_bill where ref = '%s' ";
      ...
      $sql = "update t_ws_bill
              ...
              data_org = '%s'
              where id = '%s' ";
      $rc = $db->execute($sql, $companyId, $inputUserId, $bizUserId, $dataOrg, $id);
      if ($rc === false) {
        return $this->sqlError(__METHOD__, __LINE__);
      }
  ...
  public function commitWSBill(&$params)
  {
    ...
    if ($receivingType == 0) {
      ...
      if ($totalBalanceMoney < $outMoney) {
        return $this->bad("预付款余额是{$totalBalanceMoney}元，小于销售金额，无法付款");
      }
