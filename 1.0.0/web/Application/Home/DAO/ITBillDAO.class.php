...
class ITBillDAO extends PSIBaseExDAO
{
  ...
  public function addITBill(&$bill)
  {
    ...
    $limitGoods = $toWarehouse["limitGoods"] == 1;
    $usageType = $toWarehouse["usageType"];
    ...
    foreach ($items as $i => $v) {
      ...
      if (!$goods) {
        return $this->bad("选择的物料不存在");
      }
      if ($limitGoods) {
        $recordInde = $i + 1;
        $mType = $goods["mType"];
        // 仓库启用了物料限制，检查当前物料是否是仓库允许的物料类型
        if ($usageType == 10) {
          // 原材料库
          if ($mType != 1000) {
            return $this->bad("调入仓库的用途是原材料库，第{$recordInde}条记录中的物料不是原材料，不能保存");
          }
        } else if ($usageType == 20) {
          // 半成品库
          if ($mType != 2000) {
            return $this->bad("调入仓库的用途是半成品库，第{$recordInde}条记录中的物料不是半成品，不能保存");
          }
        } else if ($usageType == 30) {
          // 产成品库
          if ($mType != 3000) {
            return $this->bad("调入仓库的用途是产成品库，第{$recordInde}条记录中的物料不是产成品，不能保存");
          }
        } else if ($usageType == 40) {
          // 商品库
          if ($mType != 4000) {
            return $this->bad("调入仓库的用途是商品库，第{$recordInde}条记录中的物料不是商品，不能保存");
          }
        } else {
          // bug
          return $this->bad("bug: usageType = {$usageType}");
        }
      }
  ...
  public function commitITBill(&$params)
  {
    ...
    foreach ($items as $i => $v) {
      ...
      if (!$data) {
        $index = $i + 1;
        return $this->bad("第{$index}条明细记录中的物料不存在，无法完成提交");
      }
      ...
      if (!$data) {
        return $this->bad("物料[$goodsCode $goodsName $goodsSpec]库存不足，无法调拨");
      }
