...
<script src="<?php echo ($uri); ?>Public/Scripts/PSI/PurchaseOrder/POMainForm.js?dt=<?php echo ($dtFlag); ?>" type="text/javascript"></script>

<!-- GoodsPrice会mix到POEditForm，所以需要放在其前面引用 -->
...
<script src="<?php echo ($uri); ?>Public/Scripts/PSI/PurchaseOrder/ChangeOrderEditForm.js?dt=<?php echo ($dtFlag); ?>"
  type="text/javascript"></script>
...
<script>
  PCL.onReady(() => {
    ...
    const permission = {
      ...
      genExcel: "<?php echo ($pGenExcel); ?>",
      confirm: "<?php echo ($pConfirm); ?>",
      genPWBill: "<?php echo ($pGenPWBill); ?>",
      showAddGoodsButton: "<?php echo ($showAddGoodsButton); ?>",
      print: "<?php echo ($pPrint); ?>",
      changeOrder: "<?php echo ($pChangeOrder); ?>"
    };

    app.add(PCL.create("PSI.PurchaseOrder.POMainForm", {
      permission
    }));
  });
</script>
